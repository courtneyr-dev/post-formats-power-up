<?php
/**
 * Plugin Name:       Chat Log – Display Conversation Transcripts
		...
		foreach ( $lines as $line_index => $line ) {
			...
			if ( preg_match( '/^([^([:]+)\s*\((\d{2}:\d{2}:\d{2}(?:[.,]\d{3})?)\):\s*(.+)$/i', $line, $matches ) ) {
				$messages[]    = array(
					'speaker'   => trim( $matches[1] ),
					'timestamp' => $matches[2],
					'body'      => chatlog_replace_emoji( trim( $matches[3] ) ),
					'type'      => 'message',
					'is_thread' => false,
				);
				$last_username = trim( $matches[1] );
				continue;
			}
